...
extern "C" {
	...
	{
		notify(_t("Please connect"), _t("to the internet..."), "", is_network_test);
	}
}
...
#if defined ALLEGRO_IPHONE || defined ALLEGRO_ANDROID || defined ALLEGRO_WINDOWS || defined ALLEGRO_UNIX
static bool find_touch(int touch_id, int *index)
{
...
static void process_touch(int x, int y, int touch_id, int type)
{
	...
	if (type == MOUSE_DOWN) {
		...
		if (find_touch(touch_id, &index)) {
			touches.erase(touches.begin() + index);
		}
#if defined ALLEGRO_WINDOWS // || defined ALLEGRO_UNIX
		// HUUUGE hack but I'm getting touches with IDs that don't exist - either a bug in Allegro or something,
		// so this is a hack that "fixes" it
		else if (touches.size() > 0)
		{
			int r = rand() % touches.size();
			touches.erase(touches.begin() + r);
		}
#endif
		/*
		else {
			...
			if (f) {
				fprintf(f, "touch id %x not found, touches.size=%d (", touch_id, touches.size());
				for (size_t i = 0; i < touches.size(); i++) {
					fprintf(f, "%x%s", touches[i].touch_id, i == touches.size()-1 ? ")" : ", ");
				}
				fclose(f);
				exit(0);
			}
...
#ifndef ALLEGRO_UNIX // touchscreen hack FIXME: do this on all platforms?
...
SteamAPI_RunCallbacks();
	...
	if (check_internet_connection) {
		if (config.getPurchased() != 1) {
			show_please_connect_dialog(true); // this returns immediately if connected, else blocks
		}
	}
#endif
...
#if defined ALLEGRO_WINDOWS || defined ALLEGRO_UNIX
		if (forced_tablet) {
			if (event.type == ALLEGRO_EVENT_TOUCH_BEGIN || event.type == ALLEGRO_EVENT_TOUCH_END || event.type == ALLEGRO_EVENT_TOUCH_MOVE) {
				my_mouse_state.x = event.touch.x;
				my_mouse_state.y = event.touch.y;
				my_mouse_state.display = display;
				my_mouse_state.buttons = 1;

			}
			else if (event.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN || event.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP || event.type == ALLEGRO_EVENT_MOUSE_AXES) {
				my_mouse_state.x = event.mouse.x;
				my_mouse_state.y = event.mouse.y;
				my_mouse_state.display = display;
				my_mouse_state.buttons = 1;
			}
		}
...
#ifdef ALLEGRO_ANDROID_XXX_NOPE
		if (gamepadConnected() && (
...
#define MOVE ALLEGRO_EVENT_MOUSE_AXES
...
#if defined ALLEGRO_WINDOWS || defined ALLEGRO_UNIX
		(event.type == ALLEGRO_EVENT_TOUCH_BEGIN || event.type == ALLEGRO_EVENT_TOUCH_END || event.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN || event.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP || event.type == ALLEGRO_EVENT_TOUCH_CANCEL ||
		((event.type == ALLEGRO_EVENT_MOUSE_AXES || event.type == ALLEGRO_EVENT_TOUCH_MOVE) && !path_head))) {
#else
		(event.type == BEGIN ||
		event.type == END ||
				...
				if (!roam || !player_in_corner) {
					...
					if (global_draw_red || red_off_press_on) {
						if (this_x < 16 && this_y < 16) {
							...
							if (al_current_time() > next_shake && (forced_tablet || !on_title_screen)) {
...
#ifdef ALLEGRO_UNIX // touchscreen hack
					if (battle == 0) {
						al_unlock_mutex(input_mutex);
						break;
					}
...
#if defined ALLEGRO_IPHONE || defined ALLEGRO_ANDROID
				if (!use_dpad) {
#elif defined ALLEGRO_WINDOWS || defined ALLEGRO_UNIX
				if ((forced_tablet && !use_dpad) || (forced_tablet == false && !released)) {
#else
				if (!released) {
#endif
					if (last_mouse_x < 0) {
		...
		else if (event.type == ALLEGRO_EVENT_DISPLAY_SWITCH_IN)
		{
			...
			*/
			...
			// hack to avoid race condition... see crystal picnic engine source for details
			al_rest(0.5);
