//#define ALLEGRO_CFG_RELEASE_LOGGING=1
...
static LONG WINAPI unhandled_exception(_EXCEPTION_POINTERS *exception_pointers)
{
	...
	snprintf(tmp, 5000, "%s/crashdump.dmp", al_path_cstr(user_path, ALLEGRO_NATIVE_PATH_SEP));
...
bool enable_press_and_hold(bool enable, ALLEGRO_DISPLAY *d)
{
#ifdef _WIN32
...
#define TABLET_ENABLE_MULTITOUCHDATA       0x01000000
	...
	if (enable) {
		...
		return RemoveProp(al_get_win_window_handle(d), tabletAtom) != 0;
	}
	else {
		// Try to disable press and hold gesture by 
		// setting the window property, return the result
		//return SetProp(al_get_win_window_handle(d), tabletAtom, (HANDLE)(TABLET_DISABLE_PRESSANDHOLD | TABLET_DISABLE_PENTAPFEEDBACK | TABLET_DISABLE_PENBARRELFEEDBACK | TABLET_DISABLE_FLICKS)) != 0;
		return SetProp(al_get_win_window_handle(d), tabletAtom, (HANDLE)1) != 0;
	}
#else
	return false;
#endif
...
ALLEGRO_MUTEX *keyboard_state_mutex;
...
bool forced_tablet = false;

ALLEGRO_MOUSE_STATE my_mouse_state;

static int drag_w = -1;
static int drag_h = -1;

bool physfs_inited = false;
...
#endif
	medium_font = my_load_ttf_font(getResource(font_name.c_str()), 32, ttf_flags);
	if (!medium_font) {
...
{
#if defined ALLEGRO_ANDROID || defined ALLEGRO_WINDOWS || defined ALLEGRO_UNIX
	...
	for (int k = 0; k < nj; k++) {
		...
		if (nb >= 6) {
			if (curr_index == config.getJoyIndex()) {
				user_joystick = j;
#if defined ALLEGRO_ANDROID || defined ALLEGRO_WINDOWS || defined ALLEGRO_UNIX
#if defined ALLEGRO_WINDOWS || defined ALLEGRO_UNIX
				if (forced_tablet)
...
static void *loader_proc(void *arg)
{
	...
	keyboard_state_mutex = al_create_mutex();
...
void init_shaders(void)
{
	...
	if (use_programmable_pipeline) {
		...
		"   if (" ALLEGRO_SHADER_VAR_USE_TEX_MATRIX ") {\n"
		"     float4 uv = mul(" ALLEGRO_SHADER_VAR_TEX_MATRIX ", float4(Input.TexCoord, 0.0, 1.0));\n"
...
//FIXME:
...
#if defined ALLEGRO_WINDOWS || defined ALLEGRO_UNIX
	if (check_arg(*argc, *argv, "-force-tablet") > 0)
#endif
	{
#if defined ALLEGRO_WINDOWS || defined ALLEGRO_UNIX
		forced_tablet = true;
    ...
    //int physfs_init_result = PHYSFS_init(al_path_cstr(exename, '/'));
    //ALLEGRO_DEBUG((std::string("exename=") + al_path_cstr(exename, '/')).c_str());
    //char physfs_buf[1000];
    //sprintf(physfs_buf, "PHYSFS_Init returned %d (0==error) error=%s", physfs_init_result, PHYSFS_getLastError());
    //ALLEGRO_DEBUG(physfs_buf);
#endif
    PHYSFS_addToSearchPath(al_path_cstr(exename, '/'), 1);
    //int physfs_result = PHYSFS_addToSearchPath(al_path_cstr(exename, '/'), 1);
#ifdef ALLEGRO_ANDROID
    //sprintf(physfs_buf, "addToSearchPath result=%d (0==fail) PHYSFS error=%s argv0=%s", physfs_result, PHYSFS_getLastError()), myArgv[0];
    //ALLEGRO_DEBUG(physfs_buf);
    //const PHYSFS_ArchiveInfo **arch_info = PHYSFS_supportedArchiveTypes();
    //for (int i = 0; arch_info[i] != NULL; i++) {
        //ALLEGRO_DEBUG(arch_info[i]->extension);
    //}
#endif
...
#ifdef ALLEGRO_WINDOWS
	enable_press_and_hold(false, display);
	set_dragsize();
#endif
	...
	if (al_is_touch_input_installed()) {
		// must check forced_tablet because if not al_install_touch_input was not called
...
#ifdef ALLEGRO_ANDROID
		//is_close_pressed(true); // pump events so we don't get an ANR
#endif
	}
...
if (g_SteamAchievements)
	...
	al_destroy_mutex(keyboard_state_mutex);
...
void resizer(bool is_toggle_fullscreen)
{
...
void set_dragsize()
{
	...
	SystemParametersInfoA(SPI_SETDRAGWIDTH, MAX(al_get_display_width(display), al_get_display_height(display)), NULL, 0);
	SystemParametersInfoA(SPI_SETDRAGHEIGHT, MAX(al_get_display_width(display), al_get_display_height(display)), NULL, 0);
#endif
}
...
#ifdef ALLEGRO_WINDOWS
	if (drag_w >= 0) {
		SystemParametersInfoA(SPI_SETDRAGWIDTH, drag_w, NULL, 0);
	}
	if (drag_h >= 0) {
		SystemParametersInfoA(SPI_SETDRAGHEIGHT, drag_h, NULL, 0);
	}
#endif
}
