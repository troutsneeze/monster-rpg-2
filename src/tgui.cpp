...
static ALLEGRO_MUTEX *event_mutex;
...
#endif
#if defined ALLEGRO_WINDOWS || defined ALLEGRO_UNIX
		if (forced_tablet == false && al_is_mouse_installed()) {
...
#define BUTTON button
	...
	while (!al_event_queue_is_empty(mouse_events)) {
		...
		if (event.type == ALLEGRO_EVENT_TOUCH_BEGIN) {
			...
			event.any.timestamp = event.touch.timestamp;
		}
		else if (event.type == ALLEGRO_EVENT_TOUCH_END) {
			event.type = ALLEGRO_EVENT_MOUSE_BUTTON_UP;
			event.mouse.x = event.touch.x;
