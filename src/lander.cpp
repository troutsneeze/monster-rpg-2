...
top:
	...
	while  (1) {
		...
		while  (!break_for_fade_after_draw && tmp_counter > 0) {
			...
			al_lock_bitmap(lander_tmp->bitmap, ALLEGRO_PIXEL_FORMAT_ANY, 0);
			al_lock_bitmap(land_mem->bitmap, ALLEGRO_PIXEL_FORMAT_ANY, 0);
			...
			for (y1 = 0, y2 = lander_y - 15; y1 < 30; y1++, y2++) {
				...
				for (x1 = 0; x1 < 30; x1++, x2++) {
					...
					if (!done) {
						...
						c1 = al_get_pixel(lander_tmp->bitmap, x1, y1);
						c2 = al_get_pixel(land_mem->bitmap, x2, y2);
