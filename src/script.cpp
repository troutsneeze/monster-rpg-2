...
static int CSetMilestone(lua_State *stack)
{
	...
	if (num >= 0 && num < MAX_MILESTONES) {
#if defined(ALLEGRO_IPHONE) || defined (ALLEGRO_MACOSX) || defined (STEAMWORKS) || defined (GOOGLEPLAY) || defined (AMAZON)
...
static int CDoItemTutorial(lua_State *stack)
{
	...
	if ((forced_tablet == false) || (forced_tablet == true && use_dpad)) {
		...
		while  (tmp_counter > 0) {
			...
			if (stage == 0) {
				...
				if (player->getInfo().equipment.rhand != -1) {
					stage++;
#if defined ALLEGRO_WINDOWS || defined ALLEGRO_UNIX
					if ((forced_tablet == false) || (forced_tablet && use_dpad)) {
#elif !defined ALLEGRO_IPHONE && !defined ALLEGRO_ANDROID
					if (true) {
#else
					if (use_dpad) {
...
static int CAddStream(lua_State *stack)
{
	...
	lua_pushlightuserdata(stack, (void *)((int64_t)s));
