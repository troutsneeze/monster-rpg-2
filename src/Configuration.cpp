...
int Configuration::getJoyIndex()
{
	return joy_index;
}

void Configuration::setJoyIndex(int i)
{
	joy_index = i;
}
...
bool Configuration::read()
{
	...
	if (isAndroidConsole()) {
#if defined GOOGLEPLAY && defined ADMOB
	...
	XMLData *joyindex = game->find("joyindex");
	...
	*/
	...
	if (xml_language) {
		...
		bool add_korean = false;
		*/
		bool langfixed = true;
		bool add_korean = true;
		/*
		if (hacks) {
			XMLData *langfix = hacks->find("rm_french_canada");
			langfixed = atoi(langfix->getValue().c_str());
			XMLData *korean = hacks->find("add_korean");
			if (korean) {
				add_korean = atoi(korean->getValue().c_str());
			}
		}
		*/
		...
		// NOTE: take care here. if adding any new languages!!! korean was added when the languages were:
		/*
		{ "English", "English" },
	...
	if (joyindex) setJoyIndex(atoi(joyindex->getValue().c_str()));
#if !(defined ADMOB && defined GOOGLEPLAY)
...
void Configuration::write()
{
	...
	XMLData* joyindex = new XMLData("joyindex", my_itoa(getJoyIndex()));
	...
	//XMLData *add_korean = new XMLData("add_korean", my_itoa(1));
