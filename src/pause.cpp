...
bool config_menu(bool start_on_fullscreen)
{
	...
	for (;;) {
		...
		if (language_toggle->getSelected() != curr_language) {
			std::string l1 = get_language_name(config.getLanguage());
			curr_language = language_toggle->getSelected();
			config.setLanguage(curr_language);
			std::string l2 = get_language_name(config.getLanguage());
			bool is_korean1 = (l1 == "Korean");
			bool is_korean2 = (l2 == "Korean");
			/*
			if (is_korean1 == is_korean2) {
				// no need to load a new font, just load the translation
				load_translation(get_language_name(config.getLanguage()).c_str());
			}
			else {
				// need to load a new font, switching between DejaVuSans and NanumGothicBold
				destroy_fonts();
				load_fonts();
			}
			*/
			// ignore above... always need to reload fonts because cached glyphs change and
			// it won't load glyphs besides the ones that are cached in load_fonts
			destroy_fonts();
			load_fonts();
			if (reset_game_center) {
...
#endif
#if defined OUYA || defined ADMOB
	if (config.getPurchased() != 1) {
#ifdef OUYA
		options.push_back("BUY FULL VERSION");
#else
		options.push_back("REMOVE ADS!");
#ifdef ALLEGRO_IPHONE
		options.push_back("Restore Purchases");
#endif
#endif
	}
#endif
...
#if defined OUYA || defined ADMOB
			else if (widget == main_menu && main_menu->getSelected() == 5) {
...
#ifdef ALLEGRO_IPHONE
			if (widget == main_menu && main_menu->getSelected() == 6) {
				restore_purchases();
				...
				if (isPurchased()) {
					...
					main_menu->removeOption(6);
					main_menu->removeOption(6);
					config.setPurchased(true);
					config.write();
					v.push_back("Purchases");
					v.push_back("restored!");
				}
				else {
					v.push_back("Couldn't restore");
					v.push_back("purchases!");
				}
				prepareForScreenGrab1();
				title_draw(bg);
